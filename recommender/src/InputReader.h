...
public:
  ...
  void parse(const std::string &mode, const char *dlm, bool skip_header, bool plus1) {
    std::ifstream fin;
    if (mode == "train") open_train_file().swap(fin);
    else
      open_test_file().swap(fin);
    ...
    StopWatch sw;
    double elapsed;
    std::cout << "read as a vector of triplets (" << mode << ") ... ";
    sw.lap();
    while (std::getline(fin, line)) {
      std::stringstream l(line);
      std::getline(l, user, *dlm);
      boost::replace_all(user, "\"", "");
      std::getline(l, item, *dlm);
      boost::replace_all(item, "\"", "");
      std::getline(l, rating, *dlm);
      boost::replace_all(rating, "\"", "");
    ...
    if (mode == "train") {
      std::cout << "Init SP matrix (alloc & set) ... ";
      sw.lap();
  ...
  void filter_user(int th) {
    ...
    StopWatch sw;
    double elapsed;
    std::cout << "filter out rarely rated user ... ";
    sw.lap();
  ...
  void filter_item(int th) {
    ...
    StopWatch sw;
    double elapsed;
    std::cout << "filter out rarely rated item ... ";
    sw.lap();
